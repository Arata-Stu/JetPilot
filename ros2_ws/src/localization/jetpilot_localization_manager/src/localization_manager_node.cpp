...
public:
  ...
  : Node("jetpilot_localization_manager"), steady_now_(std::chrono::steady_clock::now())
  {
    ...
    vslam_confirmation_timeout_sec_ = nonnegative_parameter(
      "vslam_confirmation_timeout_sec", 60.0);
    retry_backoff_sec_ = nonnegative_parameter("retry_backoff_sec", 1.0);
    const auto max_attempts_parameter = std::max<std::int64_t>(
      0, declare_parameter<std::int64_t>("max_attempts", 3));
    max_attempts_ = static_cast<int>(std::min<std::int64_t>(
      max_attempts_parameter, std::numeric_limits<int>::max()));
    poll_period_sec_ = std::max(0.01, nonnegative_parameter("poll_period_sec", 0.1));
    status_publish_period_sec_ = std::max(
      0.05, nonnegative_parameter("status_publish_period_sec", 0.5));

    validation_options_.expected_frame_id = declare_parameter<std::string>(
      "expected_pose_frame", "map");
    validation_options_.quaternion_norm_tolerance = nonnegative_parameter(
      "quaternion_norm_tolerance", 0.1);
    validation_options_.max_pose_age_sec = nonnegative_parameter("max_pose_age_sec", 0.0);

    vslam_hint_request_topic_ = declare_parameter<std::string>(
      "vslam_hint_request_topic", "/visual_slam/trigger_hint");
    vslam_pose_hint_topic_ = declare_parameter<std::string>(
    ...
    vgl_pose_topic_ = declare_parameter<std::string>(
    ...
    pose_hint_required_topic_ = declare_parameter<std::string>(
      "pose_hint_required_topic", "/localization/pose_hint_required");
    pose_hint_state_topic_ = declare_parameter<std::string>(
    ...
    pose_hint_required_pub_ = create_publisher<std_msgs::msg::Bool>(
      pose_hint_required_topic_, status_qos);
    pose_hint_state_pub_ = create_publisher<std_msgs::msg::String>(
      pose_hint_state_topic_, status_qos);
    ...
    relocalize_service_ = create_service<std_srvs::srv::Trigger>(
      ...
      std::bind(
        &LocalizationManagerNode::on_relocalize_service, this,
    ...
    RCLCPP_INFO(
      get_logger(),
      "Localization manager started (use_vgl=%s, autostart=%s, max_attempts=%d)",
...
private:
  ...
  void start_attempt()
  {
    ...
    if (max_attempts_ > 0) {
      RCLCPP_INFO(
        get_logger(), "Starting VGL localization attempt %d/%d (source=%s)",
        attempts_, max_attempts_, request_source_.c_str());
    } else {
      RCLCPP_INFO(
        get_logger(), "Starting VGL localization attempt %d (source=%s, unlimited retries)",
  ...
  void request_vgl()
  {
    ...
    try {
      ...
      const auto future_and_request_id = vgl_trigger_client_->async_send_request(
        request,
        [this, generation](rclcpp::Client<Trigger>::SharedFuture future) {
    ...
    const rclcpp::Client<Trigger>::SharedFuture & future, std::uint64_t generation)
  {
    ...
    if (!response->success) {
      RCLCPP_WARN(
        get_logger(), "VGL rejected trigger: %s", response->message.c_str());
  ...
  void schedule_retry(const std::string & reason)
  {
    ...
    RCLCPP_WARN(
      get_logger(), "Localization attempt failed (%s); retrying after %.2f s",
  ...
  void wait_for_manual(const std::string & reason)
  {
    ...
    RCLCPP_WARN(
      get_logger(), "Waiting for a manual pose on %s (%s)",
      manual_pose_topic_.c_str(), reason.c_str());
  }
  ...
  void queue_pose(const Pose & pose, const std::string & source)
  {
    ...
    set_deadline(dependency_wait_timeout_sec_);
    RCLCPP_WARN(
      get_logger(), "Holding %s pose until VSLAM is ready on %s",
      source.c_str(), vslam_pose_hint_topic_.c_str());
  }
  ...
  void forward_pending_pose()
  {
    ...
    RCLCPP_INFO(
      get_logger(), "Forwarded %s pose hint to %s",
      last_hint_source_.c_str(), vslam_pose_hint_topic_.c_str());
  }
    ...
    const Trigger::Request::SharedPtr /* request */, Trigger::Response::SharedPtr response)
  {
    ...
    response->message = use_vgl_ ?
      "Localization request accepted" : "VGL is disabled; waiting for /initialpose";
  }
    ...
    switch (state_) {
      case State::kWaitingForDependencies: {
          ...
          if (service_ready && can_forward_pose()) {
            ...
            schedule_retry(service_ready ?
              "vslam_readiness_timeout" : "vgl_service_timeout");
          } else {
            if (!service_ready) {
  ...
  void publish_status()
  {
    ...
    json << "{"
         ...
         << "\"vgl_available\":" <<
      (vgl_trigger_client_->service_is_ready() ? "true" : "false") << ","
         << "\"vgl_service_ready\":" <<
      (vgl_trigger_client_->service_is_ready() ? "true" : "false") << ","
         << "\"vslam_hint_subscriber_ready\":" <<
      (vslam_subscriber_ready() ? "true" : "false") << ","
         << "\"vslam_diagnostics_received\":" <<
      (saw_vslam_diagnostics_ ? "true" : "false") << ","
