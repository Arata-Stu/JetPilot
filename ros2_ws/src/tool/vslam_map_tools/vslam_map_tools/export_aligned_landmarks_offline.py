...
def main() -> None:
    ...
    parser.add_argument(
        "--no-path",
        action="store_true",
        help="Do not draw the saved VSLAM path or include it in automatic raster bounds.",
    )
    ...
    if args.require_landmarks and not landmarks_data:
        raise RuntimeError(
            "Snapshot does not contain landmarks. Check VSLAM visualization and the landmarks topic."
        )
    if landmarks_data:
    ...
    if args.reference_yaml:
        ...
        geometry = auto_raster_geometry(
            ...
            path_xy=np.empty((0, 2)) if args.no_path else path_xy,
