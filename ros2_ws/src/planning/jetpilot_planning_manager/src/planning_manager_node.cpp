...
RouteStamp route_stamp(const rclcpp::Time & stamp)
{
  return route_stamp(static_cast<builtin_interfaces::msg::Time>(stamp));
}
...
{
  const auto current_stamp = route_stamp(now());
...
void PlanningManagerNode::invalidate_route_bundle(const bool reset_pending_cycles)
{
  ...
  selected_lane_.clear();
  route_bundle_received_at_.reset();
  minimum_route_stamp_ = route_stamp(now());
