...
def test_launcher_resolves_project_root_when_scripts_are_mounted_separately(
    ...
    shutil.copytree(LAUNCHER.parent / "lib", detached_scripts / "lib")
    env = dict(os.environ)
...
def test_realsense_imu_defaults_on_and_d455_mount_offset_is_configured() -> None:
    realsense_source = (
        PROJECT_ROOT
    ...
    for key, value in (("enable_accel", "true"), ("enable_gyro", "true"),
                       ("unite_imu_method", "2"), ("accel_fps", "250"),
                       ("gyro_fps", "200")):
        assert f"{key}: {value}" in realsense_config
    assert "args.add_arg('enable_accel', True)" in realsense_source
    assert "args.add_arg('enable_gyro', True)" in realsense_source
    assert "args.add_arg('sensor_kit_enable_accel', True" in bringup_source
    assert "args.add_arg('sensor_kit_enable_gyro', True" in bringup_source
...
def test_rgb_evs_e2e_benchmark_uses_async_cuda_and_diagnostics_only() -> None:
    output = run_launcher(
        "rgb-evs-e2e-benchmark",
        "--e2e-model",
        "/workspaces/ros2_ws/models/e2e/test-rgb-event-async",
        "--dry-run",
    ).stdout
    benchmark_config_path = (
        PROJECT_ROOT
        / "ros2_ws/src/launch/jetpilot_system_launch/config/tool/"
        "bag_manager.rgb_evs_e2e_benchmark.param.yaml"
    )
    benchmark_config = benchmark_config_path.read_text(encoding="utf-8")
    ...
    assert "enable_vehicle:=false" in output
    assert "e2e_async_rgb_evs_mode:=true" in output
    ...
    assert "- /e2e/latent_state/diagnostics" in benchmark_config
