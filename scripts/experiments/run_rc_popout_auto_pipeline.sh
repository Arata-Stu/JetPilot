...
for session in "${sessions[@]}"; do
  ...
  if [[ -z "$raw_file" || ! -f "$raw_file.metadata.yaml" ]]; then
    ...
    record_summary "$name" FAILED yes unknown 0 0 0 0 "$auto_json" "$time_sync" "$video_file" "RAW or sidecar metadata not found"
    failed=$((failed + 1))
    continue
  fi
  if ! find "$session" -maxdepth 1 -type f -name '*.mcap' -print -quit | grep -q .; then
    echo "  FAILED: MCAP not found" >&2
    record_summary "$name" FAILED yes unknown 0 0 0 0 "$auto_json" "$time_sync" "$video_file" "MCAP not found"
    failed=$((failed + 1))
    continue
  fi
  ...
  if [[ "$needs_export" == true ]]; then
    echo "  1/3 exporting LED previews and spatial tiles"
    mkdir -p "$sync_dir"
    if ! ros2 run multi_sensor_calibration multi-sensor-calibration led-sync-export \
      ...
      record_summary "$name" FAILED yes unknown 0 0 0 0 "$auto_json" "$time_sync" "$video_file" "LED export failed"
      failed=$((failed + 1))
      continue
    fi
  else
    echo "  1/3 spatial LED data already exists"
  fi

  if [[ "$FORCE_SYNC" == true || ! -s "$auto_json" || ! -s "$time_sync" || "$data_json" -nt "$auto_json" || "$data_json" -nt "$time_sync" ]]; then
    echo "  2/3 detecting ROIs and estimating LED time synchronization"
    if ! ros2 run multi_sensor_calibration multi-sensor-calibration auto-led-sync \
      ...
      record_summary "$name" FAILED yes unknown 0 0 0 0 "$auto_json" "$time_sync" "$video_file" "automatic LED sync failed"
      failed=$((failed + 1))
      continue
    fi
  else
    echo "  2/3 automatic LED sync already exists"
  fi
...
PY
  ...
  if [[ "$NO_VIDEO" == true ]]; then
    echo "  3/3 video disabled"
  elif [[ "$needs_video" != true ]]; then
    echo "  3/3 video already exists"
  else
    if [[ -d "$video_dir" ]]; then
    ...
    echo "  3/3 rendering full RGB/event videos"
    ...
    if ! ros2 run multi_sensor_calibration multi-sensor-calibration scenario-overlay \
      ...
      record_summary "$name" FAILED "$review" "$confidence" "$matched" "$offset_ms" "$drift_ppm" "$rms_ms" "$auto_json" "$time_sync" "$video_file" "video rendering failed"
      failed=$((failed + 1))
      continue
    fi
  fi
  ...
  record_summary "$name" COMPLETE "$review" "$confidence" "$matched" "$offset_ms" "$drift_ppm" "$rms_ms" "$auto_json" "$time_sync" "$video_file" ""
