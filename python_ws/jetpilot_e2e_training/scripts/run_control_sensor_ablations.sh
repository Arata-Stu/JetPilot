...
PYTHON_BIN="${PYTHON_BIN:-$([[ -x /opt/env/bin/python ]] && echo /opt/env/bin/python || echo python3)}"
export PYTHONPATH="${PACKAGE_DIR}/src${PYTHONPATH:+:${PYTHONPATH}}"

DATASET_DIR="${1:-}"
ABLATION_NAME="${2:-control_sensor_ablation_$(date +%Y%m%d_%H%M%S)}"
EXTRA_OVERRIDES=("${@:3}")
if [[ -z "$DATASET_DIR" ]]; then
  echo "Usage: scripts/run_control_sensor_ablations.sh DATASET_DIR [ABLATION_NAME]" >&2
  exit 1
fi

OUTPUT_ROOT="${PROJECT_ROOT}/outputs/e2e_ablations/${ABLATION_NAME}/runs"
EXPERIMENTS=(
  control_pilotnet_fusion
  control_pilotnet_gru
  control_pilotnet_imu
  control_pilotnet_gru_imu
)
for experiment in "${EXPERIMENTS[@]}"; do
  run_name="${ABLATION_NAME}_${experiment}"
  echo "=== ${experiment}: ${run_name} ==="
  "$PYTHON_BIN" -m e2e_learning.cli.train \
    experiment="${experiment}" data.dataset_dir="${DATASET_DIR}" \
    run.name="${run_name}" run.output_root="${OUTPUT_ROOT}" \
    "${EXTRA_OVERRIDES[@]}"
