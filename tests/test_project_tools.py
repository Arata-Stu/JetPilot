...
class BuildTests(unittest.TestCase):
    ...
    def test_selected_build_preserves_other_outputs_and_plans_dependencies(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertNotIn('--parallel-workers', result.stdout)
            self.assertIn('CMAKE_BUILD_PARALLEL_LEVEL=2', result.stdout)
            ...
            self.assertNotIn('--parallel-workers', explicit.stdout)
            self.assertNotIn('CMAKE_BUILD_PARALLEL_LEVEL=', explicit.stdout)
...
class RepositoryTests(unittest.TestCase):
    ...
    def test_pull_is_fast_forward_only_and_dirty_checkout_is_preserved(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(self.run_repos(workspace, 'pull', 'tools/demo').returncode, 0)
            ...
            # An unrelated incoming file must not cause a dirty checkout to be skipped.
            (origin / 'other').write_text('incoming')
            self.git(origin, 'add', 'other')
            self.git(origin, 'commit', '-m', 'unrelated change')
            self.assertEqual(self.run_repos(workspace, 'pull', 'tools/demo').returncode, 0)
            self.assertEqual((checkout / 'other').read_text(), 'incoming')
            self.assertEqual((checkout / 'file').read_text(), 'uncommitted')
            # Even user-level autostash settings must not move edits out of the way.
            self.git(checkout, 'config', 'merge.autostash', 'true')
            self.git(checkout, 'config', 'pull.rebase', 'true')
            self.git(checkout, 'config', 'rebase.autostash', 'true')
            (origin / 'file').write_text('conflicting remote change')
            self.git(origin, 'commit', '-am', 'overlapping change')
            before_conflict = self.git(checkout, 'rev-parse', 'HEAD')
            self.assertNotEqual(self.run_repos(workspace, 'pull', 'tools/demo').returncode, 0)
            self.assertEqual(self.git(checkout, 'rev-parse', 'HEAD'), before_conflict)
            self.assertEqual(self.git(checkout, 'stash', 'list'), '')
            ...
            detached = self.run_repos(workspace, 'pull', 'tools/demo')
            self.assertEqual(detached.returncode, 0)
            self.assertIn('スキップ: 1', detached.stdout)
            self.assertEqual(self.git(checkout, 'rev-parse', 'HEAD'), before)

    def test_root_and_external_repos_are_updated_and_root_changes_protected(self):
        with tempfile.TemporaryDirectory() as folder:
            base = Path(folder)
            origin = base / 'origin'
            origin.mkdir()
            ...
            self.git(origin, 'config', 'user.name', 'Test')
            (origin / 'file').write_text('one')
            (origin / '.gitignore').write_text('/tools/\n/packages.lock.repos\n')
            (origin / 'packages.repos').write_text(f'repositories:\n  tools/demo:\n    type: git\n    url: {origin}\n    version: main\n')
            self.git(origin, 'add', '.')
            self.git(origin, 'commit', '-m', 'initial')
            workspace = base / 'workspace'
            self.git(base, 'clone', str(origin), str(workspace))
            self.assertEqual(self.run_repos(workspace, 'import').returncode, 0)
            child = workspace / 'tools/demo'
            status = self.run_repos(workspace, 'status')
            self.assertEqual(status.returncode, 0, status.stderr)
            self.assertIn('JetPilot (.)', status.stdout)
            self.assertIn('tools/demo', status.stdout)
            (origin / 'file').write_text('two')
            self.git(origin, 'commit', '-am', 'second')
            dry_run = self.run_repos(workspace, 'pull', '--dry-run')
            self.assertEqual(dry_run.returncode, 0, dry_run.stderr)
            self.assertEqual(dry_run.stdout.count('PLAN: git pull --ff-only'), 2)
            self.assertIn('実行予定: 2', dry_run.stdout)
            self.assertIn('dry-run・更新未実行', dry_run.stdout)
            self.assertEqual((workspace / 'file').read_text(), 'one')
            pulled = self.run_repos(workspace, 'pull')
            self.assertEqual(pulled.returncode, 0, pulled.stderr)
            self.assertIn('更新済み: 2', pulled.stdout)
            unchanged = self.run_repos(workspace, 'pull')
            self.assertEqual(unchanged.returncode, 0)
            self.assertIn('変更なし: 2', unchanged.stdout)
            self.assertEqual((workspace / 'file').read_text(), 'two')
            self.assertEqual((child / 'file').read_text(), 'two')
            (workspace / 'file').write_text('local edit')
            (origin / 'file').write_text('three')
            self.git(origin, 'commit', '-am', 'third')
            blocked = self.run_repos(workspace, 'pull')
            self.assertNotEqual(blocked.returncode, 0)
            self.assertIn('DIRTY    JetPilot (.)', blocked.stdout)
            summary = blocked.stdout.split('=== pull 結果 ===')[1]
            self.assertIn('失敗: 1', summary)
            self.assertIn('更新済み: 1', summary)
            self.assertIn('JetPilot (.)', summary.split('\n失敗:')[1])
            self.assertIn('file', summary.split('\n失敗:')[1])
            self.assertEqual((workspace / 'file').read_text(), 'local edit')
            self.assertEqual((child / 'file').read_text(), 'three')
            root_only = self.run_repos(workspace, 'status', '.')
            self.assertEqual(root_only.returncode, 0)
            self.assertNotIn('tools/demo', root_only.stdout)
            self.assertEqual(self.run_repos(workspace, 'lock').returncode, 0)
            self.assertNotIn('  .:', (workspace / 'packages.lock.repos').read_text())
            self.git(workspace, 'branch', '--unset-upstream')
            no_upstream = self.run_repos(workspace, 'pull', '.')
            self.assertNotEqual(no_upstream.returncode, 0)
            self.assertIn('upstream が未設定', no_upstream.stdout.split('=== pull 結果 ===')[1])
            import shutil
            shutil.rmtree(child)
            missing = self.run_repos(workspace, 'pull', 'tools/demo')
            self.assertNotEqual(missing.returncode, 0)
            self.assertIn('未取得です', missing.stdout.split('=== pull 結果 ===')[1])
