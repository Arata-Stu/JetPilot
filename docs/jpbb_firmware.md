...
2026-09-21、PROPO経路とJetson／teleop経路のPWMニュートラル差によって、
経路切替後に車両が発進する症状を確認した。調整前の受信機PWM実測値は、
ステアリング中央が1524 us、ESCニュートラルが1531 usだった。一方、
JPBBのホスト経路は1500 usをニュートラルとして出力する。

使用中のFutaba送信機では、次のオフセット設定で偏りを補正した。

| 対象 | Futaba設定 |
| --- | --- |
| スロットル | `F30`（Forward側へ+30） |
| ステアリング | `L24`（Left側へ24） |

これは現在の送信機・受信機・ESC・ステアリング構成に対する実車校正値である。
送信機、受信機、ESC、サーボの交換後やESC再キャリブレーション後は値を流用せず、
`/jetpilot_bridge_interface_node/rc_channels`を再計測する。PROPOとホスト経路の
切替試験は、モーター線を外すかタイヤを浮かせた状態で行う。
