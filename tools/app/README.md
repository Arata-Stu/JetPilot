...
上部メニューとホームは「Jetsonに接続」「地図を作る・直す」「走行を振り返る」
...
event window. Per-frame event count and latest-packet age remain available in
the analysis timeline for synchronization diagnosis.
...
1. Jetsonホスト名とホスト作業ディレクトリを確認。初期値は
   ...
   `10.42.0.1`、`192.168.55.1`、`192.168.11.11` と `/home/tamiya` も画面からワンクリックで選択できます。
...
2. 「環境を準備」で管理用screen内から `isaac-ros activate` を実行します。Dockerが存在する場合の接続もこのコマンドに任せます。既存の管理用screenは再利用します。
   Web UIは選択したproject rootから `ros2_ws` を解決し、非対話screenにも
   `ISAAC_ROS_WS` と `ISAAC_DIR` を明示します。以前の環境変数不足で失敗した管理用screenは、
   次の「環境を準備」で安全に作り直します。
3. 準備開始後は約30秒間、Dockerの起動を自動確認します。「SSH接続済み」「Docker起動待ち」
   「接続済み」を分けて表示します。必要に応じて「状態・ログ更新」で再確認でき、環境準備ログも表示できます。
   環境準備ログを開いている間は自動更新を止めるため、ログの選択やコピーが中断されません。
4. 用途・センサー・モデル等を選び、「起動内容を確認」「bringupを起動」を実行。
5. 「bringupを終了」はtmuxの管理対象ペインにCtrl+Cを送り、Dockerとscreenを保持します。完了は状態更新で確認してください。

bringupの初期パスは `/workspaces/scripts/bringup.sh` です。Webからは非対話CLIを使い、E2Eの制御方式は既存のmetadata判定に任せます。記録操作は `/bag/request` に送信します。送信成功は録画成功の確認ではないため、bag managerのログも確認してください。

SSH切断後もscreen・tmuxは残ります。状態・ログ更新で再取得でき、手動SSHからも同じコンテナ・ユーザーでtmuxへattachできます。tmuxは画面専用のセッション名（初期値 `jetpilot-web`）を使います。同名の手動セッションは操作せず、実行中セッションの再起動も拒否します。終了後のペインを残して終了コードとログを表示します。

JetsonホストにはPython 3・screen・Docker・isaac-ros、コンテナにはbash・tmux・ROSとビルド済みワークスペースが必要です。Web UIはDockerイメージのビルドやワークスペースの配備を行いません。`isaac-ros activate` が入力を待つ場合は環境準備ログを確認し、手動でscreenへattachしてください。起動処理に失敗したscreenも調査用に残します。

画面の「プロセス実行中」はROSトピックの正常性を保証しません。走行操作と停止は既存のJoyで行い、車両を停止してからbringupを終了してください。通信切断を検出して車両を停止する機構を、この画面で新設しているわけではありません。


### 記録状態と実行中の調整

実機画面は `/bag/status` を読み、「記録中／停止中／応答なし」と保存先を表示します。
コンテナ接続を確認した後、実機画面を開いている間は約5秒ごとに取得します。
状態は取得時点のスナップショットです。応答がない場合に「停止中」とは表示しません。

E2E出力・Joy出力の固定スロットル、ステアoffset（−1〜1）、scale（0〜3）を
現在値取得・適用できます。ステアは `入力 × scale ＋ offset` を出力範囲に制限します。
Joyにはスロットル倍率もあります。固定値は固定モードでのみ使用します。
変更は実行中のノードに反映し、次回起動用の設定ファイルには保存しません。

この機能を使う前にJetsonで `jetpilot_e2e_inference` と `jetpilot_teleop_tools` を
再ビルドし、一度bringupを起動し直してください。その後の値変更では再起動不要です。
古いノードで値だけが更新されることを避けるため、対応パラメータの宣言を確認してから変更します。

RealSense `/realsense` のRGB・Infraは、現在の解像度を保って30 / 60 / 90 Hzに変更します。
対象ストリームを一時停止し、プロファイル変更後に元のON/OFF状態へ戻します。
対応Hzは機種に依存し、変更拒否時は元の設定への復元を試みてエラーを表示します。
EVSのHz変更はこの操作の対象外です。実際の画像配信再開はライブ映像で確認してください。

地図・ライン・区間速度は「地図・ライン・区間速度の実車調整を開く」から既存画面へ移動できます。
TensorRTモデルの実行中差し替えは未実装です。モデル変更は現在の起動設定から再起動してください。

### Controllerと追従ライン

実行中の調整対象「Controller」で現在値取得・適用ができます。
`jetpilot_controller` を更新・再ビルドして一度起動し直した後、次の項目はbringup再起動なしで変更できます。

- 制御方式：pure_pursuit / map_pursuit / kinematic_mpc
- Pure/Map Pursuit：最小・最大lookahead、速度に応じたlookahead係数
- 操舵上限・変化率、Map Pursuit横偏差ゲイン、MPCの経路・方位・操舵の重み
- 速度PID、スロットルfeedforward、ブレーキP、スロットル／ブレーキ上限、目標速度上限

ノードは変更候補全体を検証してから反映します。速度制御の係数を変更するとPIDの積分履歴はリセットされます。
制御方式を切り替えてもトピック接続は維持します。起動周期・入出力トピックなどは今回の動的変更対象外です。
現在使っていない制御方式の係数は、その方式に切り替えたときに使用します。変更は設定ファイルへ保存しません。

centerline / raceline / custom lineは制御方式とは別の軌道選択です。
用途「通常走行：地図・ライン実車調整」（tuning、地図必須）で起動し、地図画面の実車調整モードで
ラインと速度を選択 → プレビュー → 実車に適用します。既存の適用条件（STOP・1秒以上停車）は維持します。
通常のcompetition起動へこの画面から別の軌道配信を割り込ませることはしません。

### 一覧の省スペース表示と実行前の確認

地図・走行解析の一覧は名前、状態、「開く」を中心に表示します。
パス、地図の生成物・生成チェック、解析の注意事項・ログ・削除は「詳細」から展開できます。
注意事項の件数と、実行中の解析の進捗・停止操作は折りたたまずに表示します。
詳細の開閉状態は、その画面を更新しても同じブラウザセッション内で維持します。

走行解析・E2E評価の開始ボタン直前に、実際の実行リクエストから作った設定要約を表示します。
走行記録、カメラ、地図、方式、抽出上限などを確認してから開始できます。

上部の共通バーには接続・プロセスと記録状態を、それぞれの確認時刻付きで表示します。
