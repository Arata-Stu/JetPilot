...
Maps → 対象Map → Simulationで経路・共通の車両設定を選び、**3方式を比較**を押します。
Pure Pursuit・Map Pursuit・Kinematic MPCが、同じ初期位置・速度0・同じ比較時間で走ります。
比較時間（1〜120秒）、初期横ずれ、初期向きのずれを指定できます。
青・橙・紫の軌跡を重ね、追従誤差RMS、最大誤差、単位時間あたりの操舵変化量、走行距離、経過時間、終了理由を表示します。
誤差は経路の点ではなく線分までの距離です。終点到達・追従不能では途中終了するため、誤差だけでなく経過時間も比較してください。
Customは各地点の速度profile、それ以外は画面の目標速度を使用します。既存の単独再生は**PPを再生**です。

比較はブラウザー内で実行され、実車の設定は変更しません。計算中は中止でき、経路・設定を変更すると古い結果を破棄します。
操舵の計算は実車のC++実装に対応し、Map Pursuitの横誤差補正・速度による操舵低減、
MPCの操舵候補ごとの予測・コスト評価を再現しています。方式固有の値は「方式別の条件」で確認できます。
