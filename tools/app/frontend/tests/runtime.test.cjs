...
vm.runInContext("runtimeResult={screen_running:false,container_running:false}",context);
assert.equal(vm.runInContext('runtimeConnectionState()[1]',context),'SSH接続済み');
vm.runInContext("runtimeResult={screen_running:true,container_running:false}",context);
assert.equal(vm.runInContext('runtimeConnectionState()[1]',context),'Docker起動待ち');
vm.runInContext("runtimeResult={screen_running:true,container_running:true}",context);
assert.equal(vm.runInContext('runtimeConnectionState()[1]',context),'接続済み');
...
(async()=>{
  ...
  context.api = async()=>({state:'not_started',message:'detected',container:'actual_isaac_container',container_running:true});
  await vm.runInContext("runtimeAction('status')",context);
  assert.equal(vm.runInContext('runtimeConfig.container',context),'actual_isaac_container');
