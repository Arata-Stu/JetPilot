...
async function refreshAll() {
  ...
  state.config = config;
  if (typeof runtimeApplyHostConfig === "function") runtimeApplyHostConfig(config);
  ...
  const availableDatasetPaths = new Set(state.e2ePipeline.datasets.map((item) => item.path));
  state.e2ePipeline.trainDatasetDirs = state.e2ePipeline.trainDatasetDirs.filter(
    (path) => availableDatasetPaths.has(path),
  );
  state.e2ePipeline.validationDatasetDirs = state.e2ePipeline.validationDatasetDirs.filter(
    (path) => availableDatasetPaths.has(path),
  );
  if (!state.e2ePipeline.trainDatasetDirs.length && state.e2ePipeline.datasetDir) {
    state.e2ePipeline.trainDatasetDirs = [state.e2ePipeline.datasetDir];
  }
...
function analysisTopics(kind) {
  ...
  const matches = {
    ...
    event: (topic) => /event_camera_msgs\/(msg\/)?EventPacket$/.test(topic.type) || /(^|\/)events(_raw)?$/i.test(topic.name),
  }[kind];
...
function preferredAnalysisTopic(kind, topics) {
  const preferences = {
    ...
    event: ["/event_camera/events", "/event_camera/events_raw"],
  }[kind] || [];
...
function e2eAnalysisPayload() {
  ...
  return {
    ...
    evaluation_dataset: mode === "supervised" ? analysis.e2eEvaluationDataset : "",
    ...
    <div class="analysis-viewer">
      <div class="analysis-viewer-title">
        ...
        <div class="chips">
          ...
          ${eventPreview.enabled ? `<span class="chip ${Number(eventPreview.generated) > 0 ? "ok" : "missing"}">${esc(eventPreview.generated || 0)} event previews</span>` : ""}
...
async function selectAnalysisBag(path) {
  ...
  try {
    ...
    for (const kind of ["image", "control", "mode", "pose", "speed", "event"]) {
...
function renderJetsonTransfers() {
  ...
  const pushMapOptions = (state.maps || [])
    .filter((map) => map?.path)
      ...
      <section class="transfer-card transfer-push">
        ...
        <div class="form-grid">
          <div class="field full">
            ...
            <input id="push-local" list="push-local-map-options" value="${esc(pushLocalPath)}" placeholder="Start typing a Map name or path" oninput="updateJetsonPushLocal(this.value)" autocomplete="off" />
