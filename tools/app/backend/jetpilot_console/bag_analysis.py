    ...
    if trajectory_mode == "offline":
        ...
        lines.extend(
            [
                ...
                '    if [[ "$nodes" != *visual_slam_node* && "$nodes" != *visual_global_localization_node* && "$nodes" != *localization_manager* && "$nodes" != *vslam_reference_snapshot_recorder* && "$nodes" != *rosbag2_player* ]]; then',
                "      return 0",
                "    fi",
                "    sleep 1",
                "  done",
                '  echo "offline ROS graph did not stop; refusing to start a second localization pipeline" >&2',
                "  return 1",
                "}",
