...
def build_analysis_script(
    ...
    *,
    ...
    if trajectory_mode == "offline":
        if map_dir is None or topic_config is None or model_dir is None:
            raise ValueError("offline localization requires map, camera topic config, and VGL model paths")
        lines.extend(
            [
                f"export ROS_DOMAIN_ID={int(config.analysis_ros_domain_id)}",
                'echo "[stage] offline VGL/VSLAM localization"',
                (
                    f"{status_writer} --set-status running --stage offline_localization "
                    f"--progress 0.05 --message {_q('Running offline VGL/VSLAM localization.')}"
                ),
                ...
                '    offline_launch_pid=""',
                '    echo "offline localization launch exited before replay became ready"',
                "    exit 1",
                "  fi",
                ...
                '  echo "offline localization readiness timed out after 180 seconds"',
                '  kill -TERM "$offline_launch_pid" 2>/dev/null || true',
                '  wait "$offline_launch_pid" 2>/dev/null || true',
                '  offline_launch_pid=""',
                "  exit 1",
                "fi",
                (
                    f"{status_writer} --set-status running --stage offline_replay "
                    f"--progress 0.15 --message {_q('Localization nodes are ready; starting paused rosbag replay.')}"
                ),
                "sleep 5",
                ...
                "done",
                (
                    f"{status_writer} --set-status running --stage offline_drain "
                    f"--progress 0.40 --message {_q('Rosbag replay finished; draining localization output.')}"
                ),
                "sleep 5",
                'kill -INT "$offline_launch_pid" 2>/dev/null || true',
                'if wait "$offline_launch_pid"; then offline_launch_status=0; else offline_launch_status=$?; fi',
                'offline_launch_pid=""',
                'if [ "$offline_launch_status" -ne 0 ] && [ "$offline_launch_status" -ne 130 ]; then',
                '  echo "offline localization launch exited with status $offline_launch_status"',
                '  exit "$offline_launch_status"',
                "fi",
                f'test -s {_q(snapshot)} || {{ echo "offline localization produced no VSLAM snapshot"; exit 1; }}',
            ]
        )
