        ...
        lines.extend(
            [
                ...
                *(
                ...
                "      offline_vgl_ready=0",
                "    fi",
                '    if [[ "$offline_nodes" == *visual_slam_node* ]] && [ "$offline_vgl_ready" -eq 1 ] && [[ "$offline_nodes" == *localization_manager* ]] && [[ "$offline_nodes" == *vslam_reference_snapshot_recorder* ]] && [[ "$offline_resume_type" == *rosbag2_interfaces/srv/Resume* ]]; then',
                "      offline_ready=1",
                "      break",
                "    fi",
                ...
                "    sleep 1",
                "  done",
                f"  if ! {status_writer} --set-status running --stage offline_drain --progress \"$offline_drain_progress\" --message \"Rosbag replay finished; draining $offline_method_label output.\"; then",
                "    offline_stop_launch TERM || true",
                "    return 27",
                "  fi",
                "  sleep 5",
                ...
                '    echo "offline $offline_method localization launch exited with status $offline_launch_status"',
                '    return "$offline_launch_status"',
                "  fi",
                f'  if [ ! -s {_q(snapshot)} ]; then echo "offline $offline_method localization produced no confirmed map-frame VSLAM snapshot"; return 28; fi',
                "  return 0",
                "}",
            ]
        )
